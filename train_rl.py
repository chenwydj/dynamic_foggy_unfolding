...
def run_training(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(args, train_loader, model, criterion, batch_criterion, optimizer, epoch)
        ...
        checkpoint_path = os.path.join(args.save_path, 'checkpoint_{:03d}.pth.tar'.format(epoch))
        save_checkpoint({
            'epoch': epoch + 1,
            'arch': args.arch,
...
def train(args, train_loader, model, criterion, batch_criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        for idx in range(len(actions)):
            for act in actions[idx]:
                rewards[idx].append((1 - act.squeeze().float()) * normalized_skip_weight)
        ...
        for idx in range(len(actions)):
            for idy in range(len(actions[0])):
                # actions[idx][idy].reinforce(args.rl_weight * cum_rewards[idx][idy].data)
                _loss = (- dists[idx][idy].log_prob(actions[idx][idy]) * (cum_rewards[idx][idy] * args.rl_weight))
        ...
        total_rewards.update(torch.cat(list(itertools.chain.from_iterable(cum_rewards.values()))).mean().data[0], input.size(0))
        total_gate_rewards = torch.cat(list(itertools.chain.from_iterable(rewards.values()))).sum().data[0]
        ...
        if i % args.print_freq == 0:
            logging.info("Epoch: [{0}][{1}/{2}]\t"
                ...
                top1=top1,
                top5=top5))
            logging.info('total gate rewards = {:.3f}'.format(total_gate_rewards))
