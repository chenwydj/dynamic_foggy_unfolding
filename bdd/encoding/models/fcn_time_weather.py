...
class FCN(BaseNet):
    ...
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.weather_classifier = FCNHead(2048, 7, norm_layer)
        self.time_classifier = FCNHead(2048, 4, norm_layer)
        if aux:
    ...
    def forward(self, x):
        ...
        weather = self.weather_classifier(c4)
        timeofday = self.time_classifier(c4)

        if self.aux:
            auxout = self.auxlayer(c3)
            auxout = upsample(auxout, imsize, **self._up_kwargs)
            outputs.append(auxout)
        return tuple(outputs), weather, timeofday
