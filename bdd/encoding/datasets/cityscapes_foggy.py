...
class CityscapesFoggy(BaseDataset):
    BASE_DIR = 'cityscapes'
    NUM_CLASS = 19
    def __init__(self, root='/ssd1/chenwy/', split='train',
                 mode=None, transform=None, target_transform=None, **kwargs):
        super(CityscapesFoggy, self).__init__(
...
def _get_cityscapes_pairs(folder, split='train'):
    ...
    if split == 'train':
        split_f = os.path.join(folder, 'train_fine_foggy_0.005.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)
    elif split == 'val':
        split_f = os.path.join(folder, 'val_fine_foggy_0.005.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)
    elif split == 'test':
