...
class BDD100K_Seg(BaseDataset):
    ...
    def __init__(self, root='/ssd1/chenwy/', split='train', mode=None, transform=None, target_transform=None, **kwargs):
        ...
        self.images, self.masks = _get_bdd100k_seg_pairs(root, split)
        if split != 'vis':
...
def _get_bdd100k_seg_pairs(folder, split='train'):
    ...
    if split == 'train':
        # img_folder = os.path.join(folder, 'seg/images/train')
        img_folder = os.path.join(folder, 'seg_luminance/0_75_r1_multiD/train')
        # img_folder = os.path.join(folder, 'seg_luminance/0_75/train')
        # img_folder = os.path.join(folder, 'seg_luminance/100_255/train')
        mask_folder = os.path.join(folder, 'seg/labels/train')
        img_paths, mask_paths = get_path_pairs(img_folder, mask_folder)
    elif split == 'val':
        # img_folder = os.path.join(folder, 'seg/images/val')
        img_folder = os.path.join(folder, 'seg_luminance/0_75_r1_multiD/val')
        # img_folder = os.path.join(folder, 'seg_luminance/0_75/val')
        # img_folder = os.path.join(folder, 'seg_luminance/100_255/val')
        mask_folder = os.path.join(folder, 'seg/labels/val')
        img_paths, mask_paths = get_path_pairs(img_folder, mask_folder)
        img_paths, mask_paths = get_path_pairs(img_folder, mask_folder)
    elif split == 'test':
