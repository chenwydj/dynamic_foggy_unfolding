...
class BDD100K_Seg(_BaseDataset):
    """
    BDD100K Segmentation dataset
    """

    def __init__(self, **kwargs):
        super(BDD100K_Seg, self).__init__(**kwargs)

    def _set_files(self):
        # ROOT: /ssd1/chenwy/bdd100k/
        # self.root = osp.join(self.root, "VOC{}".format(self.year))
        self.image_dir = osp.join(self.root, "seg_luminance/100_255")
        self.label_dir = osp.join(self.root, "seg/labels")

        if self.split in ["train", "trainval", "val", "test"]:
            self.files = []
            for filename in os.listdir(os.path.join(self.image_dir, self.split)):
    ...
    def _load_data(self, index):
        '''
        read image/label for __getitem__()
        '''
        # Set paths
        image_id = self.files[index]
        image_path = osp.join(self.root, self.image_dir, self.split, image_id + ".jpg")
        label_path = osp.join(self.root, self.label_dir, self.split, image_id + "_train_id.png")
        # Load an image
