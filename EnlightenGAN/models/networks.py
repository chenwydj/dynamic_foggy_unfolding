...
from EnlightenGAN.models.resnet import Bottleneck
from EnlightenGAN.models.danet import DANetHead
###############################################################################
# Functions
###############################################################################
...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, gpu_ids=[], skip=False, opt=None):
    ...
    elif which_model_netG == 'sid_unet_res_resize':
        netG = Unet_Res_resize_conv(opt, skip)
        ...
        if self.opt.use_norm == 1:
            ...
            if self.skip:
                if self.opt.linear_add:
                    ...
                    output = self.gamma*latent + input*self.opt.skip
                    output = output*2 - 1
                else:
                    if self.opt.latent_threshold:
                        latent = F.relu(latent)
...
class Unet_Res_resize_conv(nn.Module):
    def __init__(self, opt, skip):
        super(Unet_Res_resize_conv, self).__init__()
        ...
        self.LReLU1_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn1_1 = SynBN2d(ch) if self.opt.syn_norm else nn.BatchNorm2d(ch)
        self.conv1_2 = nn.Conv2d(ch, ch, 3, padding=p)
        self.LReLU1_2 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn1_2 = SynBN2d(ch) if self.opt.syn_norm else nn.BatchNorm2d(ch)
        self.max_pool1 = nn.AvgPool2d(2) if self.opt.use_avgpool == 1 else nn.MaxPool2d(2)

        self.conv2_1 = nn.Conv2d(ch, ch*2, 3, padding=p)
        self.LReLU2_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn2_1 = SynBN2d(ch*2) if self.opt.syn_norm else nn.BatchNorm2d(ch*2)
        self.conv2_2 = nn.Conv2d(ch*2, ch*2, 3, padding=p)
        self.LReLU2_2 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn2_2 = SynBN2d(ch*2) if self.opt.syn_norm else nn.BatchNorm2d(ch*2)
        self.max_pool2 = nn.AvgPool2d(2) if self.opt.use_avgpool == 1 else nn.MaxPool2d(2)

        self.conv3_1 = nn.Conv2d(ch*2, ch*4, 3, padding=p)
        self.LReLU3_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn3_1 = SynBN2d(ch*4) if self.opt.syn_norm else nn.BatchNorm2d(ch*4)
        self.conv3_2 = nn.Conv2d(ch*4, ch*4, 3, padding=p)
        self.LReLU3_2 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn3_2 = SynBN2d(ch*4) if self.opt.syn_norm else nn.BatchNorm2d(ch*4)
        self.max_pool3 = nn.AvgPool2d(2) if self.opt.use_avgpool == 1 else nn.MaxPool2d(2)

        self.conv4_1 = nn.Conv2d(ch*4, ch*8, 3, padding=p)
        self.LReLU4_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn4_1 = SynBN2d(ch*8) if self.opt.syn_norm else nn.BatchNorm2d(ch*8)
        self.conv4_2 = nn.Conv2d(ch*8, ch*8, 3, padding=p)
        self.LReLU4_2 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn4_2 = SynBN2d(ch*8) if self.opt.syn_norm else nn.BatchNorm2d(ch*8)
        ...
        # =============================================================================== #
        self.resblk = nn.Sequential(*[Bottleneck(ch*8, ch*8//4, norm_layer=SynBN2d if self.opt.syn_norm else nn.BatchNorm2d) for i in range(k)])
        # =============================================================================== #
        ...
        # self.deconv6 = nn.ConvTranspose2d(256, ch*4, 2, stride=2)
        self.deconv6 = nn.Conv2d(ch*8, ch*4, 3, padding=p)
        self.conv7_1 = nn.Conv2d(ch*8, ch*4, 3, padding=p)
        self.LReLU7_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn7_1 = SynBN2d(ch*4) if self.opt.syn_norm else nn.BatchNorm2d(ch*4)
        self.conv7_2 = nn.Conv2d(ch*4, ch*4, 3, padding=p)
        self.LReLU7_2 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn7_2 = SynBN2d(ch*4) if self.opt.syn_norm else nn.BatchNorm2d(ch*4)

        # self.deconv7 = nn.ConvTranspose2d(ch*4, ch*2, 2, stride=2)
        self.deconv7 = nn.Conv2d(ch*4, ch*2, 3, padding=p)
        self.conv8_1 = nn.Conv2d(ch*4, ch*2, 3, padding=p)
        self.LReLU8_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn8_1 = SynBN2d(ch*2) if self.opt.syn_norm else nn.BatchNorm2d(ch*2)
        self.conv8_2 = nn.Conv2d(ch*2, ch*2, 3, padding=p)
        self.LReLU8_2 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn8_2 = SynBN2d(ch*2) if self.opt.syn_norm else nn.BatchNorm2d(ch*2)

        # self.deconv8 = nn.ConvTranspose2d(ch*2, ch, 2, stride=2)
        self.deconv8 = nn.Conv2d(ch*2, ch, 3, padding=p)
        self.conv9_1 = nn.Conv2d(ch*2, ch, 3, padding=p)
        self.LReLU9_1 = nn.LeakyReLU(0.2, inplace=True)
        if self.opt.use_norm == 1:
            self.bn9_1 = SynBN2d(ch) if self.opt.syn_norm else nn.BatchNorm2d(ch)
        self.conv9_2 = nn.Conv2d(ch, ch, 3, padding=p)
        self.LReLU9_2 = nn.LeakyReLU(0.2, inplace=True)

        self.conv10 = nn.Conv2d(ch, 3, 1)
        if self.opt.tanh:
            self.tanh = nn.Tanh()
        
        # self.dahead = DANetHead(ch, 3, SynBN2d)
    ...
    def forward(self, input, gray, seg_mask, priors):#edges):
        flag = 0
        if input.size()[3] > 2200:
            avg = nn.AvgPool2d(2)
            input = avg(input)
        ...
        priors, pad_left, pad_right, pad_top, pad_bottom = pad_tensor(priors)
        if self.opt.self_attention:
            gray_2 = self.downsample_1(gray)
            gray_3 = self.downsample_2(gray_2)
            gray_4 = self.downsample_3(gray_3)
        ...
        if self.opt.use_norm == 1:
            if self.opt.self_attention:
                x = self.bn1_1(self.LReLU1_1(self.conv1_1(torch.cat((input, gray, seg_mask, priors), 1))))
            ...
            conv4 = self.resblk(conv4)
            conv4 = conv4*gray_4 if self.opt.self_attention else conv4

            conv4 = F.upsample(conv4, scale_factor=2, mode='bilinear')
            conv3 = conv3*gray_3 if self.opt.self_attention else conv3
            up7 = torch.cat([self.deconv6(conv4), conv3], 1)
            x = self.bn7_1(self.LReLU7_1(self.conv7_1(up7)))
            conv7 = self.bn7_2(self.LReLU7_2(self.conv7_2(x)))
            ...
            conv3 = self.resblk(conv3)
            conv3 = conv3*gray_3 if self.opt.self_attention else conv3

            conv3 = F.upsample(conv3, scale_factor=2, mode='bilinear')
            conv2 = conv2*gray_2 if self.opt.self_attention else conv2
            up8 = torch.cat([self.deconv7(conv3), conv2], 1)
            x = self.LReLU8_1(self.conv8_1(up8))
            conv8 = self.LReLU8_2(self.conv8_2(x))
